...
4. 모바일 화면 최적화: 극강의 가독성 및 띄어쓰기 원칙 (가장 기본 HTML 태그와 인라인 컬러만 허용):
   ...
   - **소제목 구분 및 간격 (제일 중요 ⭐️):** 
     소제목은 앱에서 아주 명확하게 보이도록 **위/아래로 무조건 한 줄씩 텅 빈 여백(<p><br></p>)을 두어야 합니다.**
     올바른 형태 예시:
     <p><br></p>
   ...
   - **소제목 직후 무조건 한 줄 띄우기 필수:** 소제목을 쓴 후에는 그 바로 아랫줄에 내용을 적지 못합니다. 무조건 위 예시처럼 **<p><br></p>로 텅 빈 한 줄**을 만든 후 다음 문단을 시작하세요.
   
   - **극강의 여백 및 줄 띄어쓰기 (숨통 트기):** 
     스마트폰 화면에서 글씨가 뭉치는 '벽돌' 현상을 반드시 막아야 합니다. 
     문장이 끝날 때마다 자주 문단을 쪼개고, **문단과 문단 사이에도 무조건 <p><br></p> 를 1개 이상 우겨넣어 텅텅 빌 정도로 여유롭게** 만들어주세요. 절대 문장 중간(말허리)에 줄바꿈을 하지 마세요!!
