...
`;
    ...
    if (category === 'sports') {
      try {
        const PIXABAY_API_KEY = process.env.PIXABAY_API_KEY;
        if (PIXABAY_API_KEY) {
          // 스포츠 선수의 실제 인물/액션 사진을 배경으로 쓰기 위해 Pixabay 검색
          const pixQuery = keyword.includes('오타니') || keyword.includes('야구') || keyword.includes('MLB') ? 'baseball player' : 'sports athlete';
          const pixUrl = `https://pixabay.com/api/?key=${PIXABAY_API_KEY}&q=${encodeURIComponent(pixQuery)}&image_type=photo&orientation=horizontal&safesearch=true&per_page=3`;
          const res = await fetch(pixUrl);
          const data = await res.json();
          if (data && data.hits && data.hits.length > 0) {
            bgUrlForOg = data.hits[0].largeImageURL;
          }
        }
      } catch (imgErr) {
        console.error("Pixabay API failed for sports thumbnail:", imgErr);
      }
    } else if (category === 'finance') {
      try {
        const PIXABAY_API_KEY = process.env.PIXABAY_API_KEY;
        if (PIXABAY_API_KEY) {
          const pixUrl = `https://pixabay.com/api/?key=${PIXABAY_API_KEY}&q=wall+street+finance&image_type=photo&orientation=horizontal&safesearch=true&per_page=5`;
          const res = await fetch(pixUrl);
          const data = await res.json();
          if (data && data.hits && data.hits.length > 0) {
            bgUrlForOg = data.hits[Math.floor(Math.random() * data.hits.length)].largeImageURL;
          }
        }
      } catch (imgErr) {
        console.error("Pixabay API failed for finance thumbnail:", imgErr);
      }
    }
    ...
    if (!thumbnailHtml) {
      try {
        ...
        let ogUrl = `${baseUrl}/api/og?top=${topParams}&mid=${midParams}&bottom=${bottomParams}&ext=.png`;
        if (bgUrlForOg) {
          ogUrl += `&bg=${encodeURIComponent(bgUrlForOg)}`;
        }
