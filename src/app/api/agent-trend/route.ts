...
`;
      ...
      prompt = `
당신은 대한민국 네이버 홈판(메인) 노출 알고리즘을 완벽히 간파한 최상위 '잡블로그 총괄 편집장'입니다.
우리의 궁극적인 목표는 **전문성(주제)을 버리고, 오직 트래픽(조회수)과 애드포스트 수익 극대화를 위해 대한민국 전 국민이 무조건 클릭할 수밖에 없는 어그로성 낚시/가십 글**을 기획하는 것입니다.
현재 구글 검색(Google Search)을 실시간으로 활용하여, 네이버 홈판 피드메이커(피메)에 노출되기 가장 좋은 **IT/테크 + 유명인/재벌 비하인드 융합 어그로 롱테일 키워드 5개**를 능동적으로 추론하고 발굴해내세요.
${bannedSection}

[🚨 세 번째 블로그(Blog3) 핵심 미션: 수익형 홈판 저격 알고리즘 해킹 🚨]
- 타겟팅: 전문 지식 따위는 전혀 필요 없습니다. 오직 대중의 호기심, 분노, 질투심을 자극하는 '순수 어그로/가십'을 IT, 생활, 경제와 교묘하게 엮으세요.
- 강제 키워드 규칙 (반드시 포함할 것):
   1. 대기업/재벌 오너 리스크 및 비하인드 스토리 (예: 이재용이 극대노한, OOO 딸이 아이폰 버리고 쓴다는 갤럭시 수준 ㄷㄷ, 이재용 뒷목 잡는 근황)
   2. 전 국민 공분 유발 및 팩트체크 (논란 터진 연예인 재산 분할 수준 ㄷㄷ, 세금 폭탄 꼼수)
- 제목 후킹과 팩트: 5개 키워드의 제목은 반드시 **"극대노 하는", "수준 ㄷㄷ", "뒷목 잡는", "버리고 쓴다는", "근황"** 같은 자극적인 극단적 표현을 강제로 조합하세요. 밋밋한 정보성 제목은 절대 금지합니다.

반드시 아래 JSON 형식으로만 응답하세요. 백틱(\`\`\`)이나 다른 설명은 절대 추가하지 마세요.
{
  "trends": [
    {
      "keyword": "홈판 노출을 폭발시킬 IT/가십/재벌 융합형 강력한 어그로 롱테일 키워드",
      "reason": "왜 네이버 홈판 알고리즘이 이 제목을 픽업할 수밖에 없는지(어그로 강도) 1~2문장 분석"
    }
  ]
}
`;
